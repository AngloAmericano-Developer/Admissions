...
function viewEstadofirma(){
   	$("#content").load("views/viewElectronicSignature.html?v=6.2",function(){
...
function viewStateSignature(responsfirma){
	$("#content").load("views/viewStateSignature.html?v=6.2", function(){
...
async function viewSignarure(responsfirma){
	 try {
		...
		if (!document.getElementById('estilos-dinamicos')) {
			...
			link.rel = 'stylesheet';
			link.href = 'css/signature.css?v=6.2';
...
function openModal(){
	...
	$.when(debtor,codebtor,dataGuide,student,bus_scool).done((debtor,codebtor,respGuide,respServices,bus_scool) =>{
		$("#bodyTagLarge").load("views/adminView/modalElectronicSignature.html?v=6.2", function(){
